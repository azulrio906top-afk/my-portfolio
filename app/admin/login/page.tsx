  ...
  return (
    <main className={`relative flex min-h-screen items-center justify-center overflow-hidden px-6 py-12 ${isLight ? 'bg-slate-100 text-slate-900' : 'bg-[#020817] text-slate-50'}`}>
      ...
      <div className="relative w-full max-w-md">
        ...
        <div className={`rounded-[28px] border p-7 shadow-[0_30px_80px_rgba(2,6,23,0.9)] backdrop-blur-xl ${
          isLight ? 'border-slate-200 bg-white/80' : 'border-slate-700/80 bg-slate-900/80'
        }`}>
          <div className="mb-8 flex items-center justify-between">
            ...
            <div className={`flex h-12 w-12 items-center justify-center rounded-2xl border text-lg font-semibold ${
              isLight ? 'border-sky-200 bg-sky-100 text-sky-700' : 'border-sky-400/30 bg-sky-500/10 text-sky-300'
            }`}>
              A
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div className="space-y-2">
              ...
              <input
                ...
                className={`w-full rounded-2xl border px-4 py-3 text-base placeholder:text-slate-500 transition duration-200 focus:outline-none focus:ring-2 focus:ring-sky-500/20 ${
                  isLight ? 'border-slate-300 bg-slate-50 text-slate-900 focus:border-sky-400' : 'border-slate-700 bg-slate-950/80 text-white focus:border-sky-400'
                }`}
          ...
          <div className={`mt-6 rounded-2xl border px-3 py-2 text-center text-xs ${
            isLight ? 'border-slate-200 bg-slate-50 text-slate-600' : 'border-slate-700/70 bg-slate-950/60 text-slate-400'
          }`}>
