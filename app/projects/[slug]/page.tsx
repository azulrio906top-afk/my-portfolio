...
function normalizeTags(tags: ProjectData["tags"]): string[] {
...
export default async function ProjectCaseStudy({
    ...
    return (
      <main className="min-h-screen bg-[#070b12] px-6 py-24 text-white">
        <div className="mx-auto max-w-3xl text-center">
          ...
          <Link
            href="/#work"
            className="mt-8 inline-flex items-center gap-2 rounded-full bg-sky-500 px-5 py-3 text-sm font-bold text-slate-950"
          >
   ...
   *
   * This avoids TypeScript treating `tag` as `any`
   * later in map/filter/some operations.
   */
  const tags: string[] = normalizeTags(project.tags);

  /*
   * Explicitly type the skills array.
   *
   * This fixes:
   * "Binding element 'skill' implicitly has an 'any' type."
   */
  const skills: ProjectSkill[] = project.projectSkills.map(
    (relation: ProjectSkillRelation): ProjectSkill => relation.skill
  );
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-[#070b12] text-white">
      ...
      <div className="relative mx-auto max-w-6xl px-6 py-8 sm:py-12">
          ...
          {(skills.length > 0 || tags.length > 0) && (
            <div className="mt-7 flex flex-wrap gap-2">
              ...
              {skills.map((skill: ProjectSkill) => (
                <span
                  key={`skill-${skill.id}`}
                  className="rounded-full border border-sky-400/20 bg-sky-400/10 px-3 py-1.5 text-[10px] font-bold uppercase tracking-[0.14em] text-sky-200"
                >
                  {skill.name}
                </span>
              ))}
          ...
          {project.imageUrl ? (
            <div className="max-h-[700px] overflow-auto">
              <img
                ...
                className="block min-h-full w-full object-contain"
              />
            </div>
          ) : (
            <div className="flex aspect-video items-center justify-center bg-gradient-to-br from-sky-500/20 via-slate-900 to-slate-950 text-sky-300">
        ...
        {(project.challenge || project.solution || project.architecture || project.role || project.impact) && (
          <section className="mt-10 grid gap-6 md:grid-cols-2">
            {project.challenge && (
              <article className="rounded-[28px] border border-white/10 bg-white/[0.04] p-7 sm:p-9">
                <p className="text-xs font-black uppercase tracking-[0.25em] text-sky-400">The Challenge</p>
                <p className="mt-5 text-base leading-8 text-slate-300">{project.challenge}</p>
              </article>
            )}
            {project.solution && (
              <article className="rounded-[28px] border border-white/10 bg-white/[0.04] p-7 sm:p-9">
                <p className="text-xs font-black uppercase tracking-[0.25em] text-sky-400">The Solution</p>
                <p className="mt-5 text-base leading-8 text-slate-300">{project.solution}</p>
              </article>
            )}
            {project.architecture && (
              <article className="rounded-[28px] border border-white/10 bg-white/[0.04] p-7 sm:p-9">
                <p className="text-xs font-black uppercase tracking-[0.25em] text-sky-400">Architecture</p>
                <p className="mt-5 text-base leading-8 text-slate-300">{project.architecture}</p>
              </article>
            )}
            {project.role && (
              <article className="rounded-[28px] border border-white/10 bg-white/[0.04] p-7 sm:p-9">
                <p className="text-xs font-black uppercase tracking-[0.25em] text-sky-400">My Role</p>
                <p className="mt-5 text-base leading-8 text-slate-300">{project.role}</p>
              </article>
            )}
            {project.impact && (
              <article className="rounded-[28px] border border-sky-400/20 bg-sky-400/10 p-7 sm:p-9 md:col-span-2">
                <p className="text-xs font-black uppercase tracking-[0.25em] text-sky-300">Impact</p>
                <p className="mt-5 text-base leading-8 text-sky-50">{project.impact}</p>
              </article>
            )}
          </section>
        )}
          ...
          <article className="rounded-[28px] border border-white/10 bg-white/[0.04] p-7 sm:p-9">
            ...
            <div className="mt-6 space-y-4 text-sm leading-7 text-slate-300">
              ...
              <p>
                ...
                the project is presented around a clear user
                experience and practical workflow.
              </p>
              ...
              <p>
                ...
                the technology stack covers the application
                layers represented by this build.
              </p>
              ...
              <p>
                ...
                the portfolio emphasizes reusable UI, structured
                data and production-minded implementation.
              </p>
        ...
        <footer className="mt-16 flex flex-wrap items-center justify-between gap-5 border-t border-white/10 pt-8">
          ...
          <Link
            href="/#contact"
            className="inline-flex items-center gap-2 rounded-full bg-white px-5 py-3 text-sm font-bold text-slate-950 hover:bg-slate-200"
          >
            Discuss a project
